...
import { MessageSquareText, CalendarCheck, LayoutGrid, BarChart3 } from 'lucide-react';

interface NavigationProps {
  activeTab: 'chat' | 'calendar' | 'today' | 'analytics';
  setActiveTab: (tab: 'chat' | 'calendar' | 'today' | 'analytics') => void;
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border-t border-gray-200 dark:border-gray-700 flex justify-around items-center h-16 z-50 px-2">
      ...
      <button
        ...
        className={`flex-1 flex flex-col items-center justify-center h-full transition-all duration-300 ${activeTab === 'analytics' ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300'}`}
      >
        <div className={`p-1 rounded-xl transition-all ${activeTab === 'analytics' ? 'bg-purple-50 dark:bg-purple-900/20' : ''}`}>
          <BarChart3 size={24} strokeWidth={activeTab === 'analytics' ? 2.5 : 2} />
        </div>
        <span className="text-[10px] font-medium mt-1">Аналитика</span>
      </button>
    </div>
  );
};
